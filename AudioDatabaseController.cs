    ...
    public class AudioDatabaseController 
    {
        ...
        public bool Load(string resourcesPath, out AudioDatabase database)
        {
            ...
            if (database != null)
            {
                ...
                {
                    list.Add(database);
                    Log.Info("<color=cyan><b>[GameAudioSystem.Audio.Info]</b></color> 成功加载音频库[{0}]", database.name);
                    return true;
                }
                Log.Info("<color=cyan><b>[GameAudioSystem.Audio.Info]</b></color> 音频库[{0}]已存在 无需重复加载", database.name);
                return false;
            }
            Log.Error("<color=red><b>[GameAudioSystem.Audio.Error]</b></color> 加载音频库失败 {0}", resourcesPath);
            return false;
        }
        /// <summary>
        /// 卸载音频库
        ...
        public bool Unload(string databaseName)
        {
            ...
            if (target != null)
            {
                ...
                Log.Info("<color=cyan><b>[GameAudioSystem.Audio.Info]</b></color> 成功卸载音频库[{0}]", databaseName);
                return true;
            }
            Log.Error("<color=red><b>[GameAudioSystem.Audio.Error]</b></color> 卸载音频库失败[{0}]失败 不存在", databaseName);
            return false;
        }
        /// <summary>
        /// 获取音频库
